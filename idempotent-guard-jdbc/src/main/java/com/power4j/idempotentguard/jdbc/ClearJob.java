...
public class ClearJob implements Closeable {
	...
	private final static long EXEC_DELAY_LOW = 10_000;

	private final static long EXTRA_EXPIRE_LOW = 30_000;

	private final ScheduledExecutorService executorService;

	public ClearJob(JdbcOperator jdbcOperator, long executeDelay, long extraExpire) {
		if (executeDelay < EXEC_DELAY_LOW) {
			log.warn("清理任务执行间隔过低:{} ms", executeDelay);
		}
		if (extraExpire < EXTRA_EXPIRE_LOW) {
			log.warn("额外过期清除时间过低:{} ms", extraExpire);
		}
		this.executorService = new ScheduledThreadPoolExecutor(1);
		this.executorService.schedule(() -> clear(jdbcOperator, extraExpire), executeDelay, TimeUnit.MILLISECONDS);
	}

	private void clear(JdbcOperator jdbcOperator, long extraExpire) {
		try {
			int count = jdbcOperator.clear(Duration.ofMillis(extraExpire));
